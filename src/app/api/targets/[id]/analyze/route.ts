...
export async function POST(
  ...
  try {
    const difyResult = await runDifyAnalysis(buildDifyInputs(target));
    analysisData = difyResult;
  } catch (e) {
    ...
    const result = analyzeTarget({
      ...
      recipientActions: target.recipientActions as RecipientAction[],
      recentEpisodes: target.recentEpisodes,
  ...
  const analysis = await prisma.analysisResult.upsert({
    where: { targetId: id },
    update: analysisData,
    create: { targetId: id, ...analysisData },
  });
